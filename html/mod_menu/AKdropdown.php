...
$document->addStyleSheet($tpath . '/html/mod_menu/assets/akdropdown.css'); 
...
?>
<ul id="akdropdown<?php echo $tag;?>" class="nav menu akdropdown <?php echo $class_sfx;?>"<?php
...
foreach ($list as $i => &$item) :
	...
	if ( $switch_item_deeper == true ) :
		 ...
		 if ( $menuecol == 1 )  //oeffnen wenn mehrspaltig
		  {
			  echo '<span' . $cssarrow. '></span><ul class="nav-child unstyled small dropdown-menu dropdown-level' .  $item_level . ' multicolumn-dropdown multicolumn-dropdown-'. $item_id.'">'. "\n";
			  $col                    = 0; 
			  $rowminwidth            = 0; 
			  $rowid                  = $item_id; 
		  ...
		  echo '<li><p class="h2 menutitle">'.$rowtitle.'</p></li>'. "\n";
		  }
	...
	switch ($item->level) :
		...
		case 2:  
 				 if (  $menuecol == 1 ) :
			          ...
			          if ( $open_cols == true ) { // vorherige Spalte erst schliessen.
						  echo '</ul><div class="clearfix"></div></div></div></li>'. "\n";
						  $open_cols = false; 
						  }
		...
		case 1:  
				 echo '<li'.$class.'>';
				 $rowtitle = ( $menuparams->get('headlinedropdown') == 1 ) ? $item->title : '';
				 break;
	endswitch;
	...
	switch ($item->type) :
		...
		case 'component':
		case 'heading':
			require JModuleHelper::getLayoutPath('mod_menu', 'AKdropdown_'.$item->type);
			break;

		default:
			require JModuleHelper::getLayoutPath('mod_menu', 'AKdropdown_url');
			break;
	endswitch;
