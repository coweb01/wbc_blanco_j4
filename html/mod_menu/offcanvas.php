...
<ul class="nav flex-column oc-menu<?php echo $class_sfx;?>"<?php
	$tag = '';
...
foreach ($list as $i => &$item) :
	$class = 'nav-item oc-item-'.$item->id;
	...
	if ($item->deeper) {
		if ($item->level < 2) {
			$class .= ' oc-dropdown deeper level' . $item->level;
		}
		else {
			$class .= ' oc-dropdown-submenu deeper level' . $item->level;
		}
	}
	...
	if ($item->deeper){
		echo '<ul class="nav-child unstyled small oc-dropdown-menu oc-dropdown-level' .  $item->level . '">';
	}
	elseif ($item->deeper) {
		echo '<ul>';
	}
